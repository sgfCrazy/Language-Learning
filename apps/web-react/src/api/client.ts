...
export const api = {
  async importMediaCourse(body: { title: string; type: 'audio' | 'video' | 'music'; mediaUrl?: string; transcript: string }, tokens: StoredTokens | null) {
    return getPlatformAdapter().network.post<{ packId: string; courseId: string; sentenceCount: number }>('/course-packs/import', body, { headers: authHeaders(tokens) });
  },
