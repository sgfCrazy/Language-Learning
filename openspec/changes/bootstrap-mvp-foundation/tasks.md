...
- [x] 4.1 `CourseCatalogModule`：商城列表（筛选/搜索/分页）、详情、加入学习、获取练习数据（句子懒加载） — server — 验收：supertest 覆盖筛选/搜索/加入/懒加载
- [x] 4.2 `UserCourseModule`：进度读写（课程位置/模式/句子序号），完成课程标记 — server — 验收：跨请求读写进度一致，完成课程后整体进度更新
- [x] 4.3 前端商城列表页（筛选/搜索/分页） — web/miniapp — 验收：列表渲染正确，筛选/搜索生效
- [x] 4.4 前端课程包详情页（课程列表 + 进度 + 加入/继续入口） — web/miniapp — 验收：未加入/已加入两态展示正确
- [x] 4.5 前端课程学习容器（加载句子、维护当前位置、模式切换骨架） — web/miniapp — 验收：切换到未实现模式给出提示且进度不丢

## 5. 练习引擎（practice-engine）

- [x] 5.1 拆句工具：将英文句子拆成词块（含标点独立、保留正确顺序） — shared — 验收：单元测试覆盖普通句/标点/缩写
- [x] 5.2 连词成句拼接组件（累加式片段展示、当前步骤高亮） — web/miniapp — 验收：逐步渲染与 spec PE-002 示例一致
- [x] 5.3 输入与判定：Web 键入 + 小程序点选两条交互路径，统一判定逻辑 — web/miniapp — 验收：对错判定两端一致，错误即时标红并计数
- [x] 5.4 中译英模式完整流程（中文展示、拼接、跳过标记未掌握） — web/miniapp — 验收：完成/跳过两路径都写入对应状态
- [x] 5.5 按句评分实现（正确率/用时/尝试次数） — shared — 验收：单元测试覆盖零错误快速、多次错误两档评分
- [x] 5.6 平台适配层接入：答对/答错音效与震动经适配层调用 — web/miniapp — 验收：业务代码无 `Taro.*`/`Audio` 直调

## 6. 进度追踪（progress-tracking）

- [x] 6.1 `ProgressModule`：写入练习记录端点（含离线补传去重） — server — 验收：重复上报相同 `(userId,sentenceId,clientTimestamp)` 不产生重复记录
- [x] 6.2 热力图、成长曲线、连续打卡、课程详情聚合查询端点 — server — 验收：supertest 验证聚合数值与连续打卡断日归零
- [x] 6.3 前端离线缓存与补传（适配层 storage + 恢复后批量上报） — web/miniapp — 验收：断网完成练习后联网补传成功
- [x] 6.4 前端成长页（热力图 + 成长曲线 + 课程详情） — web/miniapp — 验收：数据与接口一致，跨端切换后从服务端刷新
